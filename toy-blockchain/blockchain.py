...
class Blockchain(object):
    ...
    def add_block(self, new_block, precomputed=False):
        if not precomputed:
            ...
            for blk in self.blocks:
                if new_block.previous_hash == self.blocks[blk]['block'].hash():
                    prev_blk = self.blocks[blk]['block']
                    break
            else:
                if not new_block.previous_hash:
                    prev_blk = None
                else:
                    raise Exception("Block with previous_hash {} is not seen".format(new_block.previous_hash))

        self.blocks[new_block.identifier] = {
            'block': new_block,
            'previous_hash': new_block.previous_hash if precomputed else previous_hash,
            'previous': prev_blk if precomputed else self.head,
        }
        self.head = new_block
